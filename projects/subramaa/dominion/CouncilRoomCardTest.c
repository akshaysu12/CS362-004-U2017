...
int main() {
    ...
    printf("Expected Value: %d, ACTUAL VALUE: %d\n", preHandSize+4, state.handCount[currPlayer]);
    if (preHandSize + 4 == state.handCount[currPlayer]) {
    ...
    if (preDiscardSize+1 == state.discardCount[currPlayer]) {
      ...
      if (state.discard[currPlayer][state.discardCount[currPlayer]-1] == council_room) {
    ...
    for (int i = 0; i < numPlayers; i++) {
      if (i != currPlayer) {
        //test hand size
        if (otherPlayerHandSizes[i]+1 != state.handCount[i]) {
        ...
        //test deck size
        if (otherPlayerDeckSizes[i]-1 != state.deckCount[i]) {
